...
public class Ecore2ASDeclarationSwitch extends EcoreSwitch<Object>
{
	...
	public Object caseEClass(EClass eObject) {
		...
		String newName = AbstractEcore2AS.getOriginalName(eObject2);
...
//		}
		...
		if (eObject2.eIsSet(EcorePackage.Literals.EPACKAGE__NS_URI)) {
			if (ClassUtil.basicGetMetamodelAnnotation(eObject2) != null) {
				...
				((PackageImpl)pivotElement).setPackageId(IdManager.getRootPackageId(PivotConstants.UML_METAMODEL_NAME));
			}
			else if (eObject2 instanceof TypesPackage) {
				@SuppressWarnings("null")@NonNull String nsUri = TypesPackage.eNS_URI;
				metaModelManager.getCompleteModel().addPackageURI2completeURI(nsUri, PivotConstants.TYPES_METAMODEL_NAME);
				((PackageImpl)pivotElement).setPackageId(IdManager.getRootPackageId(PivotConstants.TYPES_METAMODEL_NAME));
			}
			else {
...
//		copyNamedElement(pivotElement, eObject2);
	...
	public Object caseETypeParameter(ETypeParameter eObject) {
		...
		converter.addMapping(eObject2, pivotElement);
		String name = AbstractEcore2AS.getOriginalName(eObject2);
...
//		}
	...
	protected @NonNull Constraint convertEOperation2Constraint(@NonNull EOperation eOperation) {
		...
		constraint.setName(AbstractEcore2AS.getOriginalName(eOperation));
...
//				constraint.setExprString(entry.getValue());
	...
	protected void copyNamedElement(@NonNull NamedElement pivotElement, @NonNull ENamedElement eNamedElement) {
		...
		String name = AbstractEcore2AS.getOriginalName(eNamedElement);
		pivotElement.setName(name);
	}
