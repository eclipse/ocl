...
public class InvalidLiteralExpImpl
	...
	public boolean validateTypeIsOclInvalid(final DiagnosticChain diagnostics, final Map<Object, Object> context)
	{
		 ...
		 *     then true
		 *     else
		 *       let status : Boolean[?] = type = OclInvalid and typeValue = null
		 *       in
		 *         'InvalidLiteralExp::TypeIsOclInvalid'.logDiagnostic(self, diagnostics, context, severity, status, 0)
		 *     endif
		 */
		...
		if (le) {
		    ...
		    try {
		        ...
		        try {
		            ...
		            final /*@Thrown*/ boolean eq = (type != null) ? (type.getTypeId() == TYP_OclInvalid.getTypeId()) : false;
		            CAUGHT_eq = eq;
		        }
		        catch (Exception e) {
		    ...
		    final /*@NonInvalid*/ boolean logDiagnostic = CGStringLogDiagnosticOperation.INSTANCE.evaluate(executor, TypeId.BOOLEAN, PivotTables.STR_InvalidLiteralExp_c_c_TypeIsOclInvalid, this, diagnostics, context, severity_0, CAUGHT_status, PivotTables.INT_0).booleanValue();
