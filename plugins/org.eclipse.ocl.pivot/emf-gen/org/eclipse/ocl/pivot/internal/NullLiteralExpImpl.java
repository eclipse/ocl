...
public class NullLiteralExpImpl
	...
	public boolean validateTypeIsOclVoid(final DiagnosticChain diagnostics, final Map<Object, Object> context)
	{
		 ...
		 *     then true
		 *     else
		 *       let status : Boolean[?] = type = OclVoid and typeValue = null
		 *       in
		 *         'NullLiteralExp::TypeIsOclVoid'.logDiagnostic(self, diagnostics, context, severity, status, 0)
		 *     endif
		 */
		...
		if (le) {
		    ...
		    try {
		        ...
		        try {
		            ...
		            final /*@Thrown*/ boolean eq = (type != null) ? (type.getTypeId() == TYP_OclVoid.getTypeId()) : false;
		            CAUGHT_eq = eq;
		        }
		        catch (Exception e) {
		    ...
		    final /*@NonInvalid*/ boolean logDiagnostic = CGStringLogDiagnosticOperation.INSTANCE.evaluate(executor, TypeId.BOOLEAN, PivotTables.STR_NullLiteralExp_c_c_TypeIsOclVoid, this, diagnostics, context, severity_0, CAUGHT_status, PivotTables.INT_0).booleanValue();
