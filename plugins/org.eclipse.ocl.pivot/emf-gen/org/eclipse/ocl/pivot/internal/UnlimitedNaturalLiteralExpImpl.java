...
public class UnlimitedNaturalLiteralExpImpl
	...
	public boolean validateTypeIsUnlimitedNatural(final DiagnosticChain diagnostics, final Map<Object, Object> context)
	{
		 ...
		 *     then true
		 *     else
		 *       let status : Boolean[?] = type = UnlimitedNatural and typeValue = null
		 *       in
		 *         'UnlimitedNaturalLiteralExp::TypeIsUnlimitedNatural'.logDiagnostic(self, diagnostics, context, severity, status, 0)
		 *     endif
		 */
		...
		if (le) {
		    ...
		    try {
		        ...
		        try {
		            ...
		            final /*@Thrown*/ boolean eq = (type != null) ? (type.getTypeId() == TYP_UnlimitedNatural.getTypeId()) : false;
		            CAUGHT_eq = eq;
		        }
		        catch (Exception e) {
		    ...
		    final /*@NonInvalid*/ boolean logDiagnostic = CGStringLogDiagnosticOperation.INSTANCE.evaluate(executor, TypeId.BOOLEAN, PivotTables.STR_UnlimitedNaturalLiteralExp_c_c_TypeIsUnlimitedNatural, this, diagnostics, context, severity_0, CAUGHT_status, PivotTables.INT_0).booleanValue();
