...
public class RealLiteralExpImpl
	...
	public boolean validateTypeIsReal(final DiagnosticChain diagnostics, final Map<Object, Object> context)
	{
		 ...
		 *     then true
		 *     else
		 *       let status : Boolean[?] = type = Real and typeValue = null
		 *       in
		 *         'RealLiteralExp::TypeIsReal'.logDiagnostic(self, diagnostics, context, severity, status, 0)
		 *     endif
		 */
		...
		if (le) {
		    ...
		    try {
		        ...
		        try {
		            ...
		            final /*@Thrown*/ boolean eq = (type != null) ? (type.getTypeId() == TYP_Real.getTypeId()) : false;
		            CAUGHT_eq = eq;
		        }
		        catch (Exception e) {
		    ...
		    final /*@NonInvalid*/ boolean logDiagnostic = CGStringLogDiagnosticOperation.INSTANCE.evaluate(executor, TypeId.BOOLEAN, PivotTables.STR_RealLiteralExp_c_c_TypeIsReal, this, diagnostics, context, severity_0, CAUGHT_status, PivotTables.INT_0).booleanValue();
