...
The standard EMF validation capabilities provide a useful overview of problems as markers in the source model, Problems View or Pop-up dialogs.

The Validity View provides a much more detailed view of the problems an so assists in debugging bad models and/or bad constaints.
...
!images/1530-validity-view.png(ValidityViewImage)!
...
The right-hand pane title Metamodel Constraint provides a Tree View of the model hierarchies that contribute constraints. An additional child element is added for each model element to which the constraint applies.
...
The pin icon toggles the track current cursor selection. When unpinned, the default, and change in mouse selection may cause recomputation of Model Elements and Metamodel Constraints contents. When pinned the contents are stable.
...
[This functionality does not appear to work in Luna SR0.]
...
The Model Elements Tool Bar is at the top and right of the left hand pane following the Model Elements. It provides facilities specific to the Model Elements.
...
Each element is preceded by a check box that enables it usage within the next validation run. All elements may be enabled or disabled using the icons in the Model Elements Tool Bar. Enabling/disabling individual elements enables/disables the element's descendants and propagates a partial enable/disable to the element's ancestors.
...
[This seems to revalidate all Model Elements regardless in Luna SR0.]
...
The constraints displayed in the right hand pane are located by constraint locators that are registered by the org.eclipse.ocl.examples.emf.validation.validity.constraint_locator extension point. A constraint locator implements the org.eclipse.ocl.examples.emf.validation.validity.locator.ConstraintLocator or the org.eclipse.ocl.examples.emf.validation.validity.ui.locator.ConstraintUILocator to define location, presentation, execution and debug launching of a particular kind of constraints.
