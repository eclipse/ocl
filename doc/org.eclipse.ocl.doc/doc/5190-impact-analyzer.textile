...
The "@ImpactAnalyzerFactory@":http://download.eclipse.org/ocl/javadoc/5.0.0/org/eclipse/ocl/examples/impactanalyzer/ImpactAnalyzerFactory.html interface allows tool builders to efficiently determine a much smaller set of model elements on which re-evaluation of expressions is necessary after a change.

Given an "OCL expression":http://download.eclipse.org/ocl/javadoc/5.0.0/org/eclipse/ocl/ecore/OCLExpression.html, the factory can be used to create an impact analyzer for a single expression as follows:
...
a "specific OCL factory":http://download.eclipse.org/ocl/javadoc/5.0.0/org/eclipse/ocl/examples/impactanalyzer/util/OCL.html can create OCL instances that ensure consistency between opposite navigation and @allInstances@ evaluation.
