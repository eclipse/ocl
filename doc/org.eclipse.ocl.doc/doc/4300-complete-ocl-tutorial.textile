...
In Luna, suitable applications are
...
Unfortunately, this failure is not shown directly in the Luna SR0 release; you can see the warning using the "Validity View":#ValidityView. It is correctly shown using Luna SR1, but also accompanied by additional warnings where the OCL from UML 2.5 is defective or exposes limitations in Eclipse UML.
...
At the time of writing, just before Luna, the underlying Papyrus problem has been cured but a minor problem remains. It is not clear whether this will be fixed in Papyrus 1.0.0.
