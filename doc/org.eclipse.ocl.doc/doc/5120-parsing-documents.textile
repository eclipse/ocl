...
As we saw in the "Parsing Constraints and Queries":#ParsingConstraints topic, the OCL parser provides an "@OCLHelper@":http://download.eclipse.org/ocl/javadoc/5.0.0/org/eclipse/ocl/helper/OCLHelper.html API for parsing constraints embedded in models.  OCL also permits constraints
...
The "@OCLInput@":http://download.eclipse.org/ocl/javadoc/5.0.0/org/eclipse/ocl/OCLInput.html class encapsulates an OCL document.  An input can be created from a string or an
input stream.
