...
This tutorial was written for Eclipse Mars: Eclipse 4.5, EMF 2.11, OCL 6.0.
...
!{width:45%}images/4500-simple-debugger-image.png(SimpleDebuggerImage)!
...
The Outline View currently displays the OCL Abstract Syntax tree; you may choose to close this view. In a future release it may change to support breakpoints.
...
The editor now highlights *.name* and *$pc* shows a PropertCallyExp as the next execution. *$pc.referredVariable* shows that it is *ecore::ENamedElement:name*.
...
The debugger shows *library.loans->select((book = self))->size() <= copies* with *library* highlighted as the next execution. The *library* is a shorthand for *self.library* so *$pc* in the Variables View shows a VariableExp for self as the next instruction.

Click F5 or *Step Into* and *$pc* advances but the editor highlight is unchanged since there is insufficient source detail to distinguish the original *self* from the subsequent *.library*.
