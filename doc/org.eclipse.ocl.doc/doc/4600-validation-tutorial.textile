...
There are 36 results in total, which is more than we want to look at, even for this very small model.
...
The *basecs in http://www.eclipse.org/ocl/3.1.0/BaseCST* root constraint similarly contributing 2 successes without problem, so we ignore it as well by unchecking the enable checkbox preceding it.
...
Having found an error of interest we can see it in context. Select the error, remove the *Show all Errors* filtering, and enable the view of all constraints by clicking the tick icon in the Model Constraint pane tool bar.
