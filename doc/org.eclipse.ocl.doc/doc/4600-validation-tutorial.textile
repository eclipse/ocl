...
This tutorial was written for Eclipse Luna: Eclipse 4.4, EMF 2.10, OCL 5.0.
...
** by model
** by metamodel
* launch an OCL debugger for a particular mode element and constraint
...
!{width:70%}images/4600-validation-view-model.png(ValidationViewModel)!
...
!{width:50%}images/4600-validation-view-model-errors.png(ValidationViewModelErrors)!

Select the *EcoreTestFile.ecore* and invoke *Validate* from the context menu. Click *OK* to dismiss.

!{width:70%}images/4600-validation-view-metamodel-errors.png(ValidationViewMetamodelErrors)!

The above results clearly show problems, but necessarily all the problems and do not show what was actually done. Sometimes validation of a model element terminates prematurely once an error has been reported. On other occasions some constraints are not run and so no corresponding errors are detected.

The above limitations are not a problem, when everything is working well, but on when you have a misunderstanding as to what is being validated a bad day can get very much worse.

h4. Validaty View Validation
...
!{width:90%}images/4600-validation-view-model-elements.png(ValidationViewModelElements)!

The right hand pane shows the root metamodel constraint sources.

!{width:90%}images/4600-validation-view-metamodel-constraints.png(ValidationViewMetamodelConstraints)!
...
You may now obtain the more detailed validation results by clicking on the Run icon in the main validation Vire tool bar.

!{width:90%}images/4600-validation-view-root-results.png(ValidationViewRootResults)!

Use the hover text to see how many validations have been rounded up into each root display.

h4. Filtering by Root Models

There are 36 results in total, which is probably more than we want to look at, even for this very small model.
...
The *basecs in http://www.eclipse.org/ocl/3.1.0/BaseCST* root constraint simularly contributing 2 successes without problem, so we ignore it as well by unchecking the enable checkbox preceding it.

The model is so simple that we can now expand it completely. Click on the + expand icons.

!{width:90%}images/4600-validation-view-some-results.png(ValidationViewSomeResults)!

You can hover over constraints to see the details and invoke *Show In Editor* you navigate to them. 

h4. Filtering by Status

The many successes are often of limited interest, so we may concentrate on Errors by invoking *Show all Errors* from the Filtering pull down towards the right of the main Valiudation View tool bar.

!{width:90%}images/4600-validation-view-error-results.png(ValidationViewErrorResults)!

Having found an error of interest we can see it in context. Select the error, remove the *Show all Errors* filtering, and enable the view of all constraint by clicking the tick icon in the Model Constraint pane tool bar.

!{width:90%}images/4600-validation-view-results-context.png(ValidationViewResultsContext)!

This shows the error, two warnings and seven successes.
...
Maybe it's time for some debugging. Select the leaf constraint below a model element, or the leaf element below a constraint and invoke *Debug Single Enabled Selection* to start the OCL debugger to step through the problematic constraint on the problematic model element. (Select the debugger perspective explicitly if it doesn'y open automatically.)

Use of the OCL debugger is described in the "Debugger Tutorial:#DebuggerTutorial.

!{width:90%}images/4600-validation-view-debugger.png(ValidationViewDebugger)!
