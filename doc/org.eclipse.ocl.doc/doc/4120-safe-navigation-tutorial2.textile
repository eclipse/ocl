...
Non-collection values may be null whenever the multiplicity is implicitly or explicitly *MyType[?]*, which permits either an instance of MyType or null. The alternative *MyType[1]* prohibits a null value. The example metamodel is compartively good with many properties such as *Loan::book* defined as as *Book[1]*. However *Loan::date* is *Date[?]* which seems unwise; why should a Loan have an unknown Date? *Book::library* is correctly *Library[?]* since there is no reason why Books may not be shelved in Bookshops or Houses. This is shown in the expanded form.

!{width:60%}images/4120-expanded-safe-navigation-error.png(Expanded Safe Navigation Error)!

@self.library.loans@ violates the UnsafeSourceCannotBeNull constraint because the source @self.library@ can be null as a consequence of the *[?]* multiplicity.

Collection values, which are almost the raison d'etreof OCL, are a disaster safe-navigation-wise. Any OCL collection may contain null values and so any OCL iteration may have a null iterator and so in the expanded example the source of @loan.book@ is unsafe.

h4. Safe Navigat Oper
