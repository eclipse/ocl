...
The OCL debugger has interpretation of the Step functionalities is adjusted to facilitate stepping to many points in a complex expression without needing to reformat the source with line breaks..
...
Execution proceeds until the next OCL AST node is associated with a reduced stack depth. Iterations and let expressions introduce nested stack entries so step return can step over an iteration. 
