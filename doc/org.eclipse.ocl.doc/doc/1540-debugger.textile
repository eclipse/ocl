...
!{width:90%}images/1540-debugger-image.png(DebuggerImage)!

The screenshot shows
* Stack Trace showing nested evaluation environments 
...
Debugging an OCL constraint requires the user to provide two pieces of information
* the expression/constraint to evaluate
...
An arbitrary OCL expression may be entered in the Xtext OCL Console and evaluated on a model object selected using a mouse selection. The Debugger adds an addition debug icon to the Console Tool Bar.
...
The Validity View provides fine-grained display of the evaluation of each constraint applicable to a model element and vice-versa. One of these model element/constraint combinations may be selected and the debugger launched to investigate why validation is producing unexpected results.

!{width:70%}images/1540-debugger-validity-view-launch.png(DebuggerValidityViewLaunch)!
...
!{width:70%}images/1540-debugger-run-configuration-launch.png(DebuggerRunConfigurationLaunch)!
...
Rather than the line based stepping typical of the Java debugger, the OCL debugger supports term by term stepping, highlighting the next term to be evaluated and showing the intermediate results. 

It is desirable to be able to step by term without modifying source text to insert line breaks. The OCL debugger therefore has a slightly different interpretation of the Step functionalities.
...
Execution proceeds until the next OCL AST node is associated with a reduced stack depth.
...
Intermediate variables are named using the property name of the subsequent AST node's input. Thus "$source" shows the OperationCallExp.source input already computed as "self.name". Multiple inputs are diasmbiguated by suffixing as in "$argument[1]".

"$pc" denotes the previous instruction. __Bug. Surely the next would be more useful?__
