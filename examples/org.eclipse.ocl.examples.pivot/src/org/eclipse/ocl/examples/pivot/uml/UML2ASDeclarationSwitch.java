...
import org.eclipse.ocl.examples.pivot.ecore.Ecore2ASDeclarationSwitch;
...
//		converter.addProfileApplication(umlProfileApplication);
		...
		if (umlMetaclass != null) {
			...
			if (umlMetapackage != null) {
				...
				if (nsURI != null) {
					metaModelManager.getCompleteModel().addPackageURI2completeURI(nsURI, DomainConstants.UML_METAMODEL_NAME);
				}
...
//		converter.addProperty(umlProperty);
	...
	public Slot caseSlot(org.eclipse.uml2.uml.Slot umlSlot) {
		...
		doSwitchAll(pivotElement.getValues(), umlSlot.getValues(), null);
				...
				if ("UML".equals(packageName)) {		// OMG's 
					for (org.eclipse.uml2.uml.Type umlType : umlPackage.getOwnedTypes()) {
						if ((umlType instanceof org.eclipse.uml2.uml.Class) && "Class".equals(umlType.getName())) {
							metaModelManager.getCompleteModel().addPackageURI2completeURI(nsURI2, DomainConstants.UML_METAMODEL_NAME);;
					...
					for (org.eclipse.uml2.uml.Type umlType : umlPackage.getOwnedTypes()) {
						if ((umlType instanceof org.eclipse.uml2.uml.PrimitiveType) && "Boolean".equals(umlType.getName())) {
							metaModelManager.getCompleteModel().addPackageURI2completeURI(nsURI2, DomainConstants.TYPES_METAMODEL_NAME);;
							break;
						}
					}
				}
...
//				umlComments.add((org.eclipse.uml2.uml.Comment)ownedElement);
		...
		if (umlOtherElements != null) {
			...
			pivotElement.getOwnedAnnotation().clear();
		}
		if (umlImportedPackages != null) {
...
// FIXME						asPackage.setNsURI(String.valueOf(value));
	...
	public <T extends Element, V extends EObject> void doSwitchAll(/*@NonNull*/ Collection<T> pivotObjects, /*@NonNull*/ List<V> eObjects, @Nullable UML2AS.Predicate<V> predicate) {
