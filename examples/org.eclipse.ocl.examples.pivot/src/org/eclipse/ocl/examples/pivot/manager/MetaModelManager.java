...
{		
	public class CompleteTypeOperationsIterable extends CompleteElementIterable<org.eclipse.ocl.examples.pivot.Class, Operation>
	{
		protected final Boolean selectStatic;	// null for static/non-static, true for static, false for non-static
		
		public CompleteTypeOperationsIterable(@NonNull Iterable<org.eclipse.ocl.examples.pivot.Class> types, boolean selectStatic) {
			super(types);
	...
	public class CompleteClassPropertiesIterable extends CompleteElementIterable<org.eclipse.ocl.examples.pivot.Class, Property>
	{
		protected final Boolean selectStatic;	// null for static/non-static, true for static, false for non-static
		
		public CompleteClassPropertiesIterable(@NonNull Iterable<org.eclipse.ocl.examples.pivot.Class> types, boolean selectStatic) {
			super(types);
			this.selectStatic = selectStatic;
		}
	...
	public class CompleteClassSuperClassesIterable
		...
		public CompleteClassSuperClassesIterable(@NonNull Iterable<org.eclipse.ocl.examples.pivot.Class> types) {
			super(types);
		}
...
//	}
	...
	public int compareOperationMatches(@NonNull Operation reference, @NonNull TemplateParameterSubstitutions referenceBindings,
		...
		Type referenceType = DomainUtil.nonNullModel(reference.getOwningClass());
		Type candidateType = DomainUtil.nonNullModel(candidate.getOwningClass());
		Type specializedReferenceType = getSpecializedType(referenceType, referenceBindings);
		Type specializedCandidateType = getSpecializedType(candidateType, candidateBindings);
		if (referenceType != candidateType) {
			if (conformsTo(specializedReferenceType, referenceBindings, specializedCandidateType, candidateBindings)) {
...
//	}
	...
	public @NonNull Iterable<Constraint> getAllInvariants(@NonNull Type pivotType) {
		...
		for (DomainClass partialSuperType : getPartialClasses(pivotType)) {
		...
		for (DomainClass superType : getAllSuperClasses(pivotType)) {
			if (superType != null) {
				for (DomainClass partialSuperType : getPartialClasses(superType)) {
					if (partialSuperType instanceof org.eclipse.ocl.examples.pivot.Class) {
						DomainPackage partialPackage = partialSuperType.getOwningPackage();
						if (!(partialPackage instanceof PackageImpl) || !((PackageImpl)partialPackage).isIgnoreInvariants()) {
							knownInvariants.addAll(((org.eclipse.ocl.examples.pivot.Class)partialSuperType).getOwnedInvariants());
...
/*	@Override
	...
	public @NonNull Iterable<? extends DomainProperty> getAllProperties(@NonNull DomainProperty pivotProperty) {
		...
		Iterable<? extends DomainProperty> memberProperties = ((CompleteClassImpl)completeClass).getMemberProperties(pivotProperty);
	...
	public @NonNull Iterable<? extends DomainClass> getAllSuperClasses(@NonNull DomainType type) {
		CompleteClass completeClass = completeModel.getCompleteClass(type);
		return completeClass.getAllSuperClasses();
	}

	@Deprecated
	public @NonNull Iterable<org.eclipse.ocl.examples.pivot.Class> getAllTypes(@NonNull org.eclipse.ocl.examples.pivot.Class pivotType) {
//		if (pivotType == null) {
...
//		return getTypeTracker(pivotType).getTypeServer().getTypes();
		...
		@SuppressWarnings("null") @NonNull Iterable<org.eclipse.ocl.examples.pivot.Class> filter = Iterables.filter(completeClass.getPartialClasses(), org.eclipse.ocl.examples.pivot.Class.class);
		return filter;
	}
...
//		return getTypeTracker(pivotType).getTypeServer().getTypes();
	...
	public @NonNull <T extends CollectionType> T getCollectionType(@NonNull T containerType, @NonNull Type elementType, @Nullable IntegerValue lower, @Nullable IntegerValue upper) {
		...
		TypeServer typeServer2 = getCompleteClass(containerType).getTypeServer();
...
//							e.printStackTrace();
	...
	public @NonNull DomainInheritance getInheritance(@NonNull DomainClass type) {
		...
		org.eclipse.ocl.examples.pivot.Class unspecializedType = (org.eclipse.ocl.examples.pivot.Class) type1.getUnspecializedElement();
		org.eclipse.ocl.examples.pivot.Class theType = unspecializedType != null ? unspecializedType : type1;
		TypeServer typeServer = getCompleteClass(theType).getTypeServer();
		return typeServer;
	}
	...
	public @Nullable org.eclipse.ocl.examples.pivot.Class getLibraryType(@NonNull String string, @NonNull List<? extends Type> templateArguments) {
...
//		assert !(libraryType instanceof CollectionType);
		...
		if (libraryTypeServer instanceof TemplateableTypeServer) {
			...
			@SuppressWarnings("unchecked") T specializedType = (T) typeServer.getSpecializedType(templateArgument, null, null);
			return specializedType;
		}
		else {
	...
	public @NonNull Iterable<CompleteClass> getLocalClasses(@NonNull org.eclipse.ocl.examples.pivot.Package pkg) {
		return getCompletePackage(pkg).getOwnedCompleteClasses();
	}

	public Iterable<Comment> getLocalComments(@NonNull Operation operation) {
		return new CompleteElementCommentsIterable(getOperationOverloads(operation));
	}
	...
	public @NonNull Iterable<Constraint> getLocalInvariants(@NonNull org.eclipse.ocl.examples.pivot.Class type) {
		type = PivotUtil.getUnspecializedTemplateableElement(type);
	...
	public @NonNull Iterable<Operation> getMemberOperations(@NonNull org.eclipse.ocl.examples.pivot.Class type, boolean selectStatic) {
		type = PivotUtil.getUnspecializedTemplateableElement(type);
	...
	public @NonNull Iterable<Property> getMemberProperties(@NonNull org.eclipse.ocl.examples.pivot.Class type, boolean selectStatic) {
		type = PivotUtil.getUnspecializedTemplateableElement(type);
...
//		}
	...
	@Override
	public org.eclipse.ocl.examples.pivot.Package getNsURIPackage(@NonNull String nsURI) {
...
//	}
	...
	public @NonNull Iterable<? extends DomainOperation> getOperationOverloads(@NonNull DomainOperation pivotOperation) {
		...
		CompleteClass completeClass = completeModel.getCompleteClass(pivotClass);
		Iterable<? extends DomainOperation> operationOverloads = ((CompleteClassImpl)completeClass).getOperationOverloads(pivotOperation);
...
//	}
	...
	public @NonNull DomainOperation getPrimaryOperation(@NonNull DomainOperation pivotOperation) {
		...
		if (pivotClass != null) {					// Null for an EAnnotation element
			CompleteClass completeClass = completeModel.getCompleteClass(pivotClass);
			DomainOperation operation = ((CompleteClassImpl)completeClass).getMemberOperation(pivotOperation);
			if (operation != null) {
				return operation;
			}
		}
	...
	public @NonNull DomainPackage getPrimaryPackage(@NonNull DomainPackage aPackage) {
		return getCompletePackage(aPackage).getPivotPackage();
...
//		}
	...
	public @NonNull DomainProperty getPrimaryProperty(@NonNull DomainProperty pivotProperty) {
		...
		if (owningInheritance != null) {
			...
			Iterable<? extends DomainProperty> memberProperties = completeClass.getAllProperties(pivotProperty.isStatic() ? FeatureFilter.SELECT_STATIC : FeatureFilter.SELECT_NON_STATIC, name);
	...
	public @NonNull org.eclipse.ocl.examples.pivot.Class getPrimaryType(@NonNull DomainClass type) {
...
//		}
	...
	public Iterable<CompleteClass> getSuperCompleteClasses(@NonNull org.eclipse.ocl.examples.pivot.Class pivotType) {
...
//		return new CompleteClassSuperClassesIterable(getAllTypes(pivotType));
	...
	public @NonNull org.eclipse.ocl.examples.pivot.Class getType(@NonNull DomainClass dType) {				// FIXME simplify eliminate
...
/*		DomainPackage dPackage = dType.getOwningPackage();	// FIXME cast
		if (dPackage != null) {
			...
			org.eclipse.ocl.examples.pivot.Class primaryType = completePackage.getMemberType(dType.getName());
			if (primaryType != null) {
				return primaryType;
			}
		}
	...
	public @NonNull Type getType(@NonNull DomainType dType) {				// FIXME simplify eliminate
		...
		if (dType instanceof PrimitiveTypeServer) {
			return ((PrimitiveTypeServer)dType).getCompleteClass().getPivotClass();
		}
	...
	public @NonNull TypeServer getTypeServer(@NonNull DomainType pivotType) {
		...
		return completeModel.getCompleteClass(pivotType).getTypeServer();
	}
	...
	public void installPropertyDeclaration(@NonNull Property thisProperty) {
		...
		org.eclipse.ocl.examples.pivot.Class thatClass = thatType.isClass();
		if ((thatClass == null) || (thatClass instanceof DataType)) {
			return;
		}
		org.eclipse.ocl.examples.pivot.Class thisClass = thisProperty.getOwningClass();
	...
	public boolean isSuperClassOf(@NonNull org.eclipse.ocl.examples.pivot.Class unspecializedFirstType, @NonNull org.eclipse.ocl.examples.pivot.Class secondType) {
...
//		org.eclipse.ocl.examples.pivot.Class unspecializedSecondType = PivotUtil.getUnspecializedTemplateableElement(secondType);	// FIXME cast
...
//		logger.warn("Cannot convert to pivot for package with URI '" + uri + "'");
	...
	public Operation resolveOperation(@NonNull org.eclipse.ocl.examples.pivot.Class leftType, @NonNull String operationName, Type... rightTypes) {
	...
	public void setASMetamodel(DomainPackage asPackage) {
		...
		if (uri != null) {
			completeModel.addPackageURI2completeURI(uri, DomainConstants.METAMODEL_NAME);
		}
	}
...
//			setDefaultStandardLibraryURI(metaNsURI);
			...
			completeModel.addPackageURI2completeURI(metaNsURI, DomainConstants.METAMODEL_NAME);
